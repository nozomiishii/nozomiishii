...
フロントエンド領域を主軸に、バックエンド・モバイル・インフラまで横断して開発できるソフトウェアエンジニアです。新規プロジェクトの立ち上げから保守運用まで一貫して経験し、銀行・製造業・スタートアップなど多様な業界で開発に携わってきました。スタートアップ案件ではチームマネジメントや技術選定もリードしています。

特に強みとするのは「開発環境をコードで設計し、品質と速度を両立させる」ことです。Lint・テスト・CI/CD・コード生成の自動化により、ある製造業プロジェクトでは4人が1日かけていた手動テストを30分に短縮しました。直近ではClaude Codeを活用し、AI支援開発の品質と再現性をチームで担保する仕組みを整えています。

電子工学からプロダクトデザインまで、コンピューターまわりのあれこれに関心があります。楽しみながら触っていけたらと思っています。
